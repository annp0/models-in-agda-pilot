...
-- concrete instance (4.1.1)
record Instance (S : GraphSig) : Set where
  open GraphSig S
  field
...
record UIDSig (S : GraphSig) : Set₁ where
  open GraphSig S
  field
...
record RetrievalSig (S : GraphSig) (U : UIDSig S) (I : Instance S) : Set₁ where
  open GraphSig S
  open UIDSig U
  open Instance I
  field
    ...
    retrNode-correct :
      ∀ (ni′ : NI) →
        let n = retrNode ni′ in (obsNS naAgg n) × (uidN n ≡ ni′)
    retrEdge-correct :
      ∀ (ei′ : EI) →
        let e = retrEdge ei′ in (obsES eaAgg e) × (uidE e ≡ ei′)
...
record MereologySig (S : GraphSig) (U : UIDSig S) (I : Instance S) : Set₁ where
  open GraphSig S
  open UIDSig U
  open Instance I
  field
    -- NM = EI-set, EM = NI-set
    -- Node Mereology: set of Edge IDs
    -- Edge Mereology: set of Node IDs
    mereoN : N → Pred EI    -- Given a station, return its incident track IDs
    mereoE : E → Pred NI    -- Given a track, return its endpoint station IDs
    ...
    -- 45. mereoE(e) ⊆ ns-IDs
    mereoN⊆AllEI :
      ∀ n → mereoN n ⊆ (λ i → Σ[ e ∈ E ] (obsES eaAgg e × uidE e ≡ i))
      -- All track IDs referenced by a station actually exist in the graph
    mereoE⊆AllNI :
      ∀ e → mereoE e ⊆ (λ i → Σ[ n ∈ N ] (obsNS naAgg n × uidN n ≡ i))
      -- All station IDs referenced by a track actually exist in the graph

-- Everything below is now parameterized by a signature, UIDs, and an instance.
module RailSystem
  (S : GraphSig)
  (U : UIDSig S)
  (I : Instance S)
  ...
  -- Euis "Track-Z99" = ⊥           -- Track-Z99's ID is in the system
  Euis : Pred EI
  ...
  -- A MereologySig instance will provide NM/EM, cardinalities, and mereoN/mereoE.
  -- We derive the “AllEI/AllNI” helpers here for reuse in constraints.
  AllEI : Pred EI
  ...
  module Paths (R : RetrievalSig S U I) (M : MereologySig S U I) where
    open RetrievalSig R
    open MereologySig M
